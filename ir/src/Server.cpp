...
void	Server::functionGenerator(const vector<string> &command, int fd){
	...
	{
		if (getClientBySocket(fd)->getPassword() == true && command.size() == 5)
		{
			if (command[1] == command[2])
			{
				sendMessage("can't be same NAME1 and NAME2\n", fd);
				return ;
			}
			this->user.push_back(command[1]);
			this->user.push_back(command[2]);
			this->user.push_back(command[3]);
			this->user.push_back(command[4]);
		}
		else 
		{
			std::cout << "bad user number" << '\n';
			exit(1);
		}
	// 	vector<string>::iterator it = this->user.begin();
	// 	for (; it != user.end(); it++)
	// 	{
	// 		cout << *it << endl;
	// 	}
